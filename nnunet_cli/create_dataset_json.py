...
def create_dataset_json(
    output_path: Path,
    dataset_name: str,
    labels_dict: dict = {"background": 0, "aorta": 1},
):
    num_training_files = len(list((output_path / "imagesTr").glob("*.nii*")))
    num_test_files = len(list((output_path / "imagesTs").glob("*.nii*")))
    dataset_json = {
        "name": dataset_name,
        "channel_names": {"0": "CT"},
        "labels": labels_dict,
    ...
    print(
        f"Contains {num_training_files} training files and {num_test_files} test files."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output path for the nnUNet dataset.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--labels-dict",
        type=str,
        default='{"background": 0, "aorta": 1}',
        help="Label dictionary as a JSON string. Default: {'background': 0, 'aorta': 1}",
    )
    ...
    if not output_path.exists():
        raise FileNotFoundError(
            f"The output path '{output_path}' does not exist. Please provide a valid path."
        )

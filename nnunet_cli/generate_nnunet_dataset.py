...
For example:
    python -m nnunet_cli.generate_nnunet_dataset \
  --images-dir ~/Documents/UPenn/image_segmentation/3d_CT \
  --segmentations-dir ~/Documents/UPenn/CT/3d_segmentations \
  --labels 7 \
  --output-path ~/Documents/UPenn/image_segmentation/nnUNet/nnUNet_raw \
...
from nnunet_cli.utils.label_utils import validate_label_consistency


def main():
    parser = argparse.ArgumentParser(
        description="Generate nnUNet folder structure for a 3D dataset extracted from 4D CT images."
    )
    parser.add_argument(
        "--output-path",
        type=str,
        required=True,
        help="Output path for the generated nnUNet folder.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Labels to extract from the segmentations, space-separated.",
    )
    parser.add_argument(
        "--labels-dict",
        type=str,
        required=True,
        help='The labels dictionary to use in the dataset.json file. Should be a JSON string with label names and their corresponding values. For example: {"background": 0, "aorta": 1}.',
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of patient-level cross-validation folds. Set to 1 for a single train/val split (default: 1).",
    )
    parser.add_argument(
        "--verify-labels",
        action="store_true",
        help="Verify segmentation labels match labels argument.",
    )
    ...
    # Validate n-folds argument
    if n_folds < 1:
        raise ValueError("Argument '--n-folds' must be >= 1 (got {}).".format(n_folds))
    ...
    if not images_dir.exists():
        raise FileNotFoundError(
            f"The 3D images directory '{images_dir}' does not exist. Please provide a valid path."
        )
    if not segmentations_dir.exists():
        raise FileNotFoundError(
            f"The 3D segmentations directory '{segmentations_dir}' does not exist. Please provide a valid path."
        )
    ...
    if not match:
        raise ValueError(
            "Dataset name must follow the format 'DatasetXXX_Name', e.g. 'Dataset001_AortaCT'"
        )
    dataset_num = match.group(1)
    if not dataset_num:
        raise ValueError(
            f"Dataset name '{dataset_name}' does not contain a valid dataset number. Please provide a name in the format 'DatasetXXX'."
        )

    # Validate labels passed in
    try:
    ...
    if args.verify_labels:
        validate_label_consistency(set(labels), segmentations_dir)
    ...
    if dataset_output_path.exists():
        if args.force:
            print(
                f"⚠️ Output path '{dataset_output_path}' exists. Deleting it because --force was specified..."
            )
            shutil.rmtree(dataset_output_path)
        else:
            print(
                f"❌ Output path '{dataset_output_path}' already exists. Use --force to overwrite."
            )
            sys.exit(1)
    ...
    create_nnunet_folder_structure(
        images_dir, segmentations_dir, dataset_output_path, labels
    )

    # Create patient-level splits for cross-validation
    print("Creating patient-level splits...")
    create_splits_for_training_and_validation_files(
        dataset_output_path / "imagesTr",
        dataset_output_path / "splits_final.json",
        n_folds=n_folds,
    )
    ...
    # Validate the dataset (Can comment out as it's faster to complete validation on Tethy's - including for documentation)

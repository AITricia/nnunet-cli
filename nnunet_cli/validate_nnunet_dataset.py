...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the nnUNet dataset directory.",
    )
    parser.add_argument(
        "--dataset-num",
        type=str,
        required=True,
        help="Dataset number (e.g., 1 for Dataset1).",
    )
    ...
    if not dataset_dir.exists():
        raise FileNotFoundError(
            f"The dataset directory '{dataset_dir}' does not exist. Please provide a valid path."
        )

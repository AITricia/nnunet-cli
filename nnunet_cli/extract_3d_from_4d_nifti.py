...
def extract_3d_from_4d_nifti(
    input_nifti: Path, output_nifti: Path, time_index: int = None, dry_run: bool = False
):
    """
    ...
    if len(file_metadata) < 4:
        raise ExtractionError(
            "Filename does not contain enough metadata for patient ID and timepoint."
        )
    ...
    if len(data_4d.shape) != 4:
        raise ExtractionError(
            f"Error: Input file '{input_nifti}' is not a 4D NIfTI file. Shape: {data_4d.shape}"
        )
    ...
    if not (0 <= time_index < max_index):
        raise ExtractionError(
            f"Time index {time_index} is out of range. Valid range: 0 to {max_index - 1}."
        )
    ...
    if dry_run:
        print(
            f"Dry run: Would extract time index {time_index} from '{input_nifti}' and save to '{output_path}'."
        )
        return
    ...
    print(
        f"✅ Successfully extracted time index {time_index} from '{input_nifti}' and saved to '{output_path}'."
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract a specific time frame (3D) from a 4D NIfTI segmentation file."
    )
    parser.add_argument(
        "--input", type=str, help="Path to the 4D NIfTI segmentation file"
    )
    parser.add_argument(
        "--output",
        type=str,
        help="Output path for the extracted 3D NIfTI segmentation file",
    )
    parser.add_argument(
        "--time-index",
        type=int,
        default=None,
        help="If not provided, will automatically extract time index from the segmentation file name.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="If set, will not perform the extraction, just print the intended output path.",
    )
    ...
    if not input_nifti.exists():
        raise FileNotFoundError(
            f"❌ Errord: Input file '{input_nifti}' does not exist."
        )
    ...
    try:
        extract_3d_from_4d_nifti(
            input_nifti, output_nifti, time_index, dry_run=args.dry_run
        )

...
def create_splits_for_training_and_validation_files(image_dir: Path, output_path: Path, n_folds: int = 5):
    # === LOAD FILENAMES AND EXTRACT PATIENT IDS ===
    # Assumes files are like: bavcta001_001.nii.gz, bavcta002_002.nii.gz

    file_list = sorted(set(image_dir.glob("*.nii")).union(image_dir.glob("*.nii.gz")))

    df = pd.DataFrame({'filename': [f.stem[:-9] for f in file_list]})
...
def main():
    ...
    """
    parser = ArgumentParser(description="Generate patient-level splits for nnUNet training.")
    parser.add_argument("--image-dir", type=str, required=True, help="Path to the directory containing training images.")
    parser.add_argument("--n-folds", type=int, default=5, help="Number of folds for cross-validation (default: 5).")
    parser.add_argument("--output-path", type=str, required = True, help="Directory where splits_final.json will be saved.")

    args = parser.parse_args()
    image_dir = Path(args.image_dir)

    if not image_dir.exists():
        raise FileNotFoundError(f"The specified image directory '{image_dir}' does not exist. Please provide a valid path.")
    if not image_dir.is_dir():
        raise NotADirectoryError(f"The specified image directory '{image_dir}' is not a directory. Please provide a valid directory.")
    ...
    if output_path.exists():
        print(f"⚠️ Warning: The output file '{output_path}' already exists. It will be overwritten.")
    create_splits_for_training_and_validation_files(image_dir, output_path, n_folds=n_folds)
